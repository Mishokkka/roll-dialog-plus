...
Архитектурные решения описаны в `docs/ARCHITECTURE.md`, а история изменений находится в `CHANGELOG.md`. `npm run smoke` автоматически пропускается, если Playwright или Chromium не установлены; путь к браузеру можно передать через `FBLRP_CHROMIUM`.
